...
handoff_id: HO-20260816-015843
updated_at: 2026-08-16T01:58:43+08:00
updated_by: /root
state: active
...
- M3-05 remains active on `chore/m3-05-performance-benchmarks` with unique draft PR #63 linked to Issue #22. Correction `809a7d26e67185a515c3a9fcf4eeb619eaf708ee` and coordination head `193e124804d161b38ddf9c0fc82c2f15d5a84cdb` passed Governance `31895635883`, Ubuntu/Windows Build `31895635802`, and Host benchmark `31895635768`; both platforms passed two fixed runs, all budgets and the unchanged 10% repeatability gate. KVM `31895635822` exposed missing fixed-package visibility; `951a79a8cc63a304ecd59dac78f0540c6075fb0f` closed it, and replacement KVM `31896362140` then rejected AGP's default `.test -> main` wiring as `NOT-SELF-INSTRUMENTING`. Correction `143d7fe8a66e0517858d78d3bfeb71921b745197` enabled AGP's documented self-instrumenting property and embedded a copied form of the pinned/locked/checksummed Kotlin 2.4.10 stdlib solely into the independently installed test APK. Exact-head KVM `31898342753` proved package visibility, self-instrumentation and Kotlin startup all closed, then failed the first real trace because the intentionally empty fixture called `reportFullyDrawn()` outside any enclosing `Choreographer#doFrame`; `1eacabe01fd488918e5349aa83784b0feaabf371` closed the copied-stdlib configuration-cache capture and moved the marker into a posted animation frame. Exact-head KVM `31899258583` passed API 29 job `95047313610` and again proved the API 36 build/install boundary, but its first trace showed that the posted empty UI frame still had no associated RenderThread frame. The current final bounded correction registers a one-shot `OnPreDrawListener` so `reportFullyDrawn()` is emitted inside the decor's first actual rendering traversal while preserving the separately recorded interactive timestamp. No threshold, sample count, product Runtime, HIGH trigger, visible fixture content or public API changes. Only compile/static gates precede the final API 36 replacement. Successful Host results and API 29 KVM are inherited. Exactly one API 29 ARM64 physical run follows only if API 36 is stable. No local emulator will be started. `stash@{0}` remains a recovery backup until merge.
