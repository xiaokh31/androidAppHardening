...
端到端启动归因按 ADR 0016 使用 common `p0..p15` 相邻时间线，并在 protected 的 `p0..p1` 内使用 `p0,h0,h1..h7,h8,p1` 嵌套时间线；`h1..h7` 保留 M2-10 `t0..t6` 语义。所有时间戳必须来自同一 boot-time monotonic trace clock，全部相邻差值必须无遗漏调和到 `p8-p0` 与 `p15-p0`。Runtime 只拥有 `h0..h8`，`platform_pre_shell`、`platform_post_loader`、生命周期、traversal 与 rendering 不得改名为 Runtime。后续独立诊断只允许 `kotlin-multidex`、同一 API 36 job/boot 的 A/B 反序、每 mode 5 次预热与 15 个保留样本；两 campaign 均满足 30 ms、10% 与 50% 归因条件才可选择 owner，否则固定为 `UNATTRIBUTED`。M2-10 run `32099991400` 不重跑、不替换；M3-09 不运行 benchmark、KVM 或 ARM。
