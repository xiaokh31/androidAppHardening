...
M0 基础建设与可行性验证已经完成，项目正在顺序执行 M1 Host 处理链。当前仍未提供端到端可用的 APK 加固程序：M1-01 输入检查、M1-02 signer policy、M1-03 Binary AXML 转换器和 M1-07 AHDC v2 分块认证容器合同已经合并并通过各自门禁；下一项开发任务是从最新 `main` 重新启动 M1-04 容器实现。
...
| [M1-04](docs/tasks/M1-04-encrypted-dex-container.md) | 下一开发任务 | 从包含 M1-07 的最新 `main` 重新实现 AHDC v2；不得复用已废止的 AHDC v1 候选 |
