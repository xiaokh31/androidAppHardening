...
handoff_id: HO-20260816-012539
updated_at: 2026-08-16T01:25:39+08:00
updated_by: /root
state: active
...
- M3-05 remains active on `chore/m3-05-performance-benchmarks` with unique draft PR #63 linked to Issue #22. Correction `809a7d26e67185a515c3a9fcf4eeb619eaf708ee` and coordination head `193e124804d161b38ddf9c0fc82c2f15d5a84cdb` passed Governance `31895635883`, Ubuntu/Windows Build `31895635802`, and Host benchmark `31895635768`; both platforms passed two fixed runs, all budgets and the unchanged 10% repeatability gate. KVM `31895635822` exposed missing fixed-package visibility; `951a79a8cc63a304ecd59dac78f0540c6075fb0f` closed it, and replacement KVM `31896362140` then rejected AGP's default `.test -> main` wiring as `NOT-SELF-INSTRUMENTING`. Correction `994df46a47d8773b08065f565f2497cd278d0548` made package and targetPackage equal, but exact-head KVM `31897250764` proved the manually equal IDs alone left target-provided Kotlin runtime out of the standalone test APK (`NoClassDefFoundError: kotlin.jvm.internal.Intrinsics`). The bounded correction enables AGP's documented `android.experimental.self-instrumenting` property and embeds a copied form of the already pinned/locked/checksummed Kotlin 2.4.10 stdlib solely into the independently installed test APK; the empty main APK remains uninstalled and available for artifact-boundary inspection. Local forced assembly proves the test APK contains `Lkotlin/jvm/internal/Intrinsics;`, package equals targetPackage, and only the three fixed fixture queries are declared. M3-07's 10 production-surface plus 20 report negatives, governance and diff checks pass without suppressing either remote failure. Only the failed API 36 job is replaced; API 29 KVM `95042335681`, successful Host/Build/Governance, and prior static evidence are inherited. Exactly one API 29 ARM64 physical run follows only if API 36 is stable. No local emulator will be started. `stash@{0}` remains a recovery backup until merge.
