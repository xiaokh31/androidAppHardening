...
const PROFILE_ARCHIVE = "21816d2a843bb5c59902224c7bf786d546d52b4a5b2d1168ca0c449a2ca27964";
...
function fail(message) {
  throw new Error(`M3-14 validation failed: ${message}`);
}
...
function validateEnvironmentLock(lockFile, currentJobFile, currentJobsPageFile, identity) {
  ...
  if (String(job.id) !== String(identity.jobId) || String(job.run_id) !== String(identity.runId) ||
      job.name !== "m3-13-startup-attribution" || job.status !== "in_progress" || job.conclusion !== null ||
  ...
  const matches = page.jobs.filter((candidate) => String(candidate.id) === String(identity.jobId) &&
    String(candidate.run_id) === String(identity.runId) && candidate.name === "m3-13-startup-attribution");
...
function validateCurrentRunsPage(file, identity) {
  ...
  const run = matches[0];
  if (String(run.id) !== String(identity.runId) || run.path !== ".github/workflows/m3-13-startup-attribution.yml" ||
      run.head_sha !== identity.headSha || run.run_attempt !== 1 || run.event !== "push" ||
      run.name !== RUN_NAME || run.status !== "in_progress" || run.conclusion !== null) {
...
const EXECUTION_IDENTITY_KEYS = [
  "schemaVersion", "contractIdentitySha256", "productTupleSha256", "profileArchiveSha256",
  "implementationFreezeSha", "diagnosticWorkflowCandidateSha256", "evidenceWorkflowCandidateSha256",
  "runnerSha256", "verifierSha256", "environmentLockSha256", "toolchainLockSha256",
  "qualificationEvidenceSha256",
];
const LEDGER_KEYS = [
  "schemaVersion", "executionIdentity", "executionIdentitySha256", "taskKey", "workflowPath",
  "workflowName", "jobName", "artifactName",
];
const EXECUTION_FILES = Object.freeze({
  diagnosticWorkflowCandidateSha256: "tools/validation/m3-14/workflow-candidates/m3-13-startup-attribution.yml",
  evidenceWorkflowCandidateSha256: "tools/validation/m3-14/workflow-candidates/m3-13-startup-attribution-evidence.yml",
  runnerSha256: "tools/validation/run-m3-14-startup-attribution.mjs",
  verifierSha256: "tools/validation/verify-m3-14-startup-attribution.mjs",
  environmentLockSha256: "tools/validation/m3-10/api36-environment-lock.json",
  toolchainLockSha256: "tools/validation/m3-14/toolchain-lock.json",
  qualificationEvidenceSha256: "docs/evidence/M3-14/qualification-evidence.json",
});

function validateExecutionLedgerDocument(ledger) {
  exactKeys(ledger, LEDGER_KEYS, "execution ledger");
  if (JSON.stringify(Object.keys(ledger.executionIdentity ?? {})) !== JSON.stringify(EXECUTION_IDENTITY_KEYS)) {
    fail("execution identity key order differs");
  }
  const identity = ledger.executionIdentity;
  if (ledger.schemaVersion !== 1 || identity.schemaVersion !== 1 ||
      identity.contractIdentitySha256 !== CONTRACT_IDENTITY ||
      identity.productTupleSha256 !== PRODUCT_TUPLE || identity.profileArchiveSha256 !== PROFILE_ARCHIVE ||
      !/^[0-9a-f]{40}$/u.test(identity.implementationFreezeSha)) {
    fail("execution identity constants differ");
  }
  for (const key of EXECUTION_IDENTITY_KEYS.slice(5)) {
    if (!/^[0-9a-f]{64}$/u.test(identity[key] ?? "")) fail(`execution identity hash differs: ${key}`);
  }
  const computed = sha256Bytes(Buffer.from(JSON.stringify(identity)));
  if (ledger.executionIdentitySha256 !== computed ||
      ledger.taskKey !== TASK_KEY || ledger.workflowPath !== ".github/workflows/m3-13-startup-attribution.yml" ||
      ledger.workflowName !== RUN_NAME || ledger.jobName !== "m3-13-startup-attribution" ||
      ledger.artifactName !== "m3-13-startup-attribution-raw") {
    fail("execution ledger binding differs");
  }
  return identity;
}

function gitText(args, label) {
  const result = spawnSync("git", args, { cwd: process.cwd(), encoding: "utf8", timeout: 30_000 });
  if (result.status !== 0) fail(`${label} Git check failed`);
  return result.stdout.trim();
}

function validateExecutionLedger(options) {
  const ledgerPath = path.resolve(required(options, "ledger"));
  const publicationHead = required(options, "publication-head");
  if (!/^[0-9a-f]{40}$/u.test(publicationHead) || publicationHead !== gitText(["rev-parse", "HEAD"], "publication head")) {
    fail("publication head differs");
  }
  const ledger = json(ledgerPath);
  const identity = validateExecutionLedgerDocument(ledger);
  const parents = gitText(["rev-list", "--parents", "-n", "1", publicationHead], "publication ancestry").split(/\s+/u);
  if (parents.length !== 2 || parents[1] !== identity.implementationFreezeSha) fail("publication is not a direct child of the reviewed freeze");
  const changed = gitText(["diff", "--name-only", `${identity.implementationFreezeSha}..${publicationHead}`], "publication diff")
  ...
  if (JSON.stringify(changed) !== JSON.stringify(allowed)) fail("publication changed paths differ");
  for (const [key, relative] of Object.entries(EXECUTION_FILES)) {
    if (identity[key] !== sha256File(path.resolve(relative))) fail(`reviewed execution file differs: ${relative}`);
  }
  const qualification = json(path.resolve(EXECUTION_FILES.qualificationEvidenceSha256));
  exactKeys(qualification, ["schemaVersion", "taskId", "issue", "implementationBaseSha", "requiredAncestors",
    "requiredHistoricalObjects", "contracts", "qualification"], "qualification evidence");
  if (qualification.schemaVersion !== 1 || qualification.taskId !== "M3-14" || qualification.issue !== 82 ||
      qualification.implementationBaseSha !== "960eb9f406eb1a7b7c9b324598fb59936aa1c5b5" ||
      qualification.contracts?.contractIdentitySha256 !== CONTRACT_IDENTITY ||
      qualification.contracts?.productTupleSha256 !== PRODUCT_TUPLE ||
      qualification.contracts?.profileArchiveSha256 !== PROFILE_ARCHIVE ||
      qualification.qualification?.fullHistoryRequired !== true ||
      qualification.qualification?.beforeAndroidRequired !== true ||
      qualification.qualification?.runLimit !== 1 || qualification.qualification?.runAttempt !== 1 ||
      qualification.qualification?.furtherRenewalPermitted !== false) fail("qualification evidence differs");
  for (const ancestor of qualification.requiredAncestors ?? []) {
    gitText(["cat-file", "-e", `${ancestor}^{commit}`], "required ancestor object");
    gitText(["merge-base", "--is-ancestor", ancestor, qualification.implementationBaseSha], "required ancestor relation");
  }
  for (const historical of qualification.requiredHistoricalObjects ?? []) {
    gitText(["cat-file", "-e", `${historical}^{commit}`], "required historical object");
  }
  gitText(["merge-base", "--is-ancestor", qualification.implementationBaseSha, identity.implementationFreezeSha], "implementation base relation");
  for (const binding of [
    [qualification.contracts.m313EligibilityLockPath, qualification.contracts.m313EligibilityLockBytes, qualification.contracts.m313EligibilityLockSha256],
    [qualification.contracts.m313OfficialProofPath, qualification.contracts.m313OfficialProofBytes, qualification.contracts.m313OfficialProofSha256],
    [qualification.contracts.m312RetentionLockPath, qualification.contracts.m312RetentionLockBytes, qualification.contracts.m312RetentionLockSha256],
  ]) {
    const file = path.resolve(binding[0]);
    if (!statSync(file).isFile() || statSync(file).size !== binding[1] || sha256File(file) !== binding[2]) {
      fail(`qualification source differs: ${binding[0]}`);
    }
  }
  for (const [live, candidate] of [
    [".github/workflows/m3-13-startup-attribution.yml", EXECUTION_FILES.diagnosticWorkflowCandidateSha256],
    [".github/workflows/m3-13-startup-attribution-evidence.yml", EXECUTION_FILES.evidenceWorkflowCandidateSha256],
  ]) {
    if (!readFileSync(path.resolve(live)).equals(readFileSync(path.resolve(candidate)))) fail(`published workflow differs from candidate: ${live}`);
  }
  return { executionIdentitySha256: ledger.executionIdentitySha256, implementationFreezeSha: identity.implementationFreezeSha };
}

function validateLedger(ledger, identity) {
  const executionIdentity = validateExecutionLedgerDocument(ledger);
  if (executionIdentity.productTupleSha256 !== identity.productTuple) fail("ledger product tuple differs");
}
...
function selfTest() {
  ...
  const currentJob = { id: 2002, run_id: 1001, name: "m3-13-startup-attribution", status: "in_progress",
  ...
  const currentRun = { id: 1001, path: ".github/workflows/m3-13-startup-attribution.yml", head_sha: identity.headSha,
    run_attempt: 1, event: "push", name: RUN_NAME, status: "in_progress", conclusion: null };
  ...
  const testExecutionIdentity = {
    schemaVersion: 1,
    contractIdentitySha256: CONTRACT_IDENTITY,
    productTupleSha256: PRODUCT_TUPLE,
    profileArchiveSha256: PROFILE_ARCHIVE,
    implementationFreezeSha: "f".repeat(40),
    diagnosticWorkflowCandidateSha256: "1".repeat(64),
    evidenceWorkflowCandidateSha256: "2".repeat(64),
    runnerSha256: "3".repeat(64),
    verifierSha256: "4".repeat(64),
    environmentLockSha256: "5".repeat(64),
    toolchainLockSha256: "6".repeat(64),
    qualificationEvidenceSha256: "7".repeat(64),
  };
  const ledger = { schemaVersion: 1, executionIdentity: testExecutionIdentity,
    executionIdentitySha256: sha256Bytes(Buffer.from(JSON.stringify(testExecutionIdentity))), taskKey: TASK_KEY,
    workflowPath: ".github/workflows/m3-13-startup-attribution.yml",
    workflowName: RUN_NAME, jobName: "m3-13-startup-attribution", artifactName: "m3-13-startup-attribution-raw" };
  validateExecutionLedgerDocument(ledger);
  const executionLedgerMutations = [
    ["execution-contract", (value) => { value.executionIdentity.contractIdentitySha256 = "8".repeat(64); }],
    ["execution-product", (value) => { value.executionIdentity.productTupleSha256 = "8".repeat(64); }],
    ["execution-profile", (value) => { value.executionIdentity.profileArchiveSha256 = "8".repeat(64); }],
    ["execution-freeze", (value) => { value.executionIdentity.implementationFreezeSha = "8".repeat(39); }],
    ["execution-candidate", (value) => { value.executionIdentity.diagnosticWorkflowCandidateSha256 = "z".repeat(64); }],
    ["execution-runner", (value) => { value.executionIdentity.runnerSha256 = "z".repeat(64); }],
    ["execution-toolchain", (value) => { value.executionIdentity.toolchainLockSha256 = "z".repeat(64); }],
    ["execution-qualification", (value) => { value.executionIdentity.qualificationEvidenceSha256 = "z".repeat(64); }],
    ["execution-hash", (value) => { value.executionIdentitySha256 = "8".repeat(64); }],
    ["execution-run-name", (value) => { value.workflowName += "-retry"; }],
  ].map(([name, mutate]) => {
    const value = structuredClone(ledger); mutate(value);
    if (name !== "execution-hash") value.executionIdentitySha256 = sha256Bytes(Buffer.from(JSON.stringify(value.executionIdentity)));
    return expectRejected(name, () => validateExecutionLedgerDocument(value));
  });
  ...
  const githubMutations = [
    ["github-ledger-tuple", (l) => { l.executionIdentity.productTupleSha256 = "d".repeat(64); }],
  ...
  return { canonical: 1, rejectedMutations: rejected + cleanupMutations.length + githubMutations.length + archiveMutations.length + resultMutations.length + environmentMutations.length + officialPageMutations.length + currentRunPageMutations.length + trackedLockMutations.length + executionLedgerMutations.length + 1,
    reportMutations: rejected, resultMutations, thresholdMutations, cleanupMutations, environmentMutations,
    officialPageMutations, currentRunPageMutations, trackedLockMutations, executionLedgerMutations, tupleMutation, githubMutations, archiveMutations,
...
function profileSelfTest(options) {
  ...
  if (!(scratch + path.sep).startsWith(allowed)) fail("profile self-test scratch must remain under build/m3-14");
  rmSync(scratch, { recursive: true, force: true });
  mkdirSync(scratch, { recursive: true });
  const mutations = [];
...
try {
  ...
  else if (command === "execution-ledger") result = validateExecutionLedger(options);
  else fail("usage: self-test | profile-self-test | profile-lock | surface | apk-pair | signed-copy | summarize | package | github-evidence | execution-ledger");
