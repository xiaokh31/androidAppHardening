...
| [M2-10](docs/tasks/M2-10-runtime-startup-performance.md) | 进行中 | Issue #66；按 ADR 0016 先做测试专用阶段归因，再只优化一个已证明的 Runtime 热点；不改变安全控制、预算或样本数，本任务不运行 ARM 或 M3-05 A/B |
...
| [M3-05](docs/tasks/M3-05-size-startup-memory-benchmarks.md) | 阻塞 | PR #63 的最终 ADR 0015 A/B 在两个顺序均超过启动预算且 25/90 重复性行失败；等待 M2-10 合并，ARM 与 M4 保持禁止 |
