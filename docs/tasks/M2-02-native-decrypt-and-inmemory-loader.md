...
- `public final class ah.runtime.loader.AuthenticatedPayloadMetadata` 只能由 `PayloadRuntime` 从同 handle 已认证快照和本次成功 package binding 构造。跨模块访问器精确固定为 `public String originalFactoryClassNameOrNull()`（无原 Factory 时唯一返回 `null`，否则返回已认证严格 FQCN）、`public int containerMajor()`、`public int containerMinor()`、`public int signerPolicyVersion()`、`public int riskPolicyVersion()`（四个版本均为无符号 16-bit 值映射到 `0..65535`）、`public byte[] buildId()`、`public byte[] keySlotId()`（各恰为 16 bytes）、`public byte[] packageNameSha256()`、`public byte[] currentSignerSha256()`（各恰为 32 bytes）和 `public byte[][] signerLineageSha256()`（旧到新、`1..16` 项、每项 32 bytes）。所有数组及嵌套数组每次返回深副本；对象不含 `R/R_java/R_native`、CEK/KEK、nonce、wrapped CEK ciphertext/tag 或原始 ConfigV2 bytes。构造器不公开，调用方不能伪造；M2-03 不得从 `UntrustedPayloadBinding` 构造安全配置。
- provisional `InMemoryDexClassLoader` 构造本身不构成 bootstrap 发布，但从构造直到 M2-03 返回完整 `VerifiedPayloadSession` 前，M2-02/M2-03 均不得调用其 `loadClass`/资源查找、解析 payload 类、注册回调或暴露 loader/metadata；原 Factory 始终在后续 M2-01 阶段才实例化。
