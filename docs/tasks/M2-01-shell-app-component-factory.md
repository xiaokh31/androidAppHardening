...
- 固定 Java package 为 `ah.runtime`，入口类为 `HardeningAppComponentFactory`。
- 模块路径固定为 `runtime/bootstrap`，Android Runtime 源码位于 `src/main/java` 并使用 Java 17；不得应用 Kotlin Android plugin。
- `instantiateClassLoader` 只调用一次 `HardeningBootstrap.install(...)`，成功后返回 M2-02 提供的 payload `ClassLoader`；状态机固定为 `NEW`、`INSTALLING`、`READY`、`FAILED`。
- Manifest 元数据键固定为 `aah.originalApplication`、`aah.originalAppComponentFactory`、`aah.containerAsset` 和 `aah.metadataVersion`。
...
- `public final class HardeningAppComponentFactory extends AppComponentFactory`
...
- `final class BootstrapResult`，通过 `Status.READY`、`Status.FAILURE`、payload `ClassLoader` 和稳定错误码表达结果。
...
- `runtime/bootstrap/src/main/java/ah/runtime/HardeningAppComponentFactory.java`
- `runtime/bootstrap/src/main/java/ah/runtime/HardeningBootstrap.java`
- `runtime/bootstrap/src/main/java/ah/runtime/BootstrapResult.java`
- `runtime/bootstrap/src/test/`
- `runtime/bootstrap/src/androidTest/`

## Dependencies and Blockers

M0-05 未证明自定义工厂、Provider 与 JNI 场景可行，或 M1-03 未冻结元数据键时不得开始实现。若公共 API 无法满足既定启动顺序，必须提交阻塞交接并回到 ADR 0003 评审，不得自行引入隐藏 API。

## Agent Handoff Requirements

使用分支 `feat/m2-01-shell-app-component-factory`，只处理 Issue `M2-01` 并仅创建一个对应 PR。交接包必须列出变更文件、关键决策、全部命令与退出码、测试设备、失败用例、产物 SHA-256、残余风险和后续 M2-02/M2-03 接口；不得修改根 `HandOff.md`。
