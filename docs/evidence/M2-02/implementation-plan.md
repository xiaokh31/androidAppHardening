...
The approved Android NDK `29.0.14206865` exposes public zlib but no public AES-GCM, HMAC-SHA-256 or HKDF API, and its sysroot contains neither OpenSSL headers nor a linkable `libcrypto`. The repository has no fixed, licensed and audited Native cryptography dependency. M2-02 requires CEK/KEK/record keys to remain Native-only, so Java/JCA is not an allowed fallback; Android's platform BoringSSL is private API, and implementing a cipher locally is prohibited.

M2-02 therefore remains blocked before production implementation. The smallest safe prerequisite is a separate ADR/task-contract and supply-chain change that selects an established Native cryptography library, pins an immutable source/version and SHA-256, records license/NOTICE and vulnerability review, defines static-link and symbol-hiding rules, and proves the NIST AES-256-GCM plus RFC 5869 vectors on all four ABIs. That prerequisite must merge before this branch resumes.
