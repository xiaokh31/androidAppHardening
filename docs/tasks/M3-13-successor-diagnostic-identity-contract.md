...
- `docs/evidence/M3-13/predecessor-official-proof.json` and its six retained raw official API pages under `docs/evidence/M3-13/raw/`.
...
- Bind retained raw official predecessor run/job-step/artifact pages, zero-observation facts and terminal history.
...
- The contract identity is SHA-256 `580560859af80418058a088c6be3f7ab221e0ab37e21d76f19bf9177be35a419` of the exact 1033-byte lock preimage, which includes the official-proof canonical SHA-256.
- A future execution identity binds the contract identity, product tuple, profile archive, reviewed implementation freeze, two non-executable workflow-candidate hashes, runner/verifier hashes, environment/toolchain locks and full-history qualification evidence. It exists only in the pre-run ledger and artifact manifest, not in candidate bytes or the run name.
- The canonical run name binds only the fixed task key, contract identity and product tuple. This removes candidate-hash self-reference while exact path/name/event/branch uniqueness preserves the one-run entitlement.
...
- Retained raw official predecessor and successor GitHub API response bytes remain part of terminal evidence. Raw job logs are excluded because step states plus reviewed control flow are sufficient and logs may expose paths.
...
- The official proof binds six exact raw API response files, contains all 17 diagnostic-job and all 10 terminal-job steps, and its SHA-256 is bound into the exact 1033-byte lock preimage, which recomputes to the fixed contract SHA-256.
...
- Raw-page mutations for diagnostic/terminal run, job-step and artifact facts; historical Git-object byte/hash verification; sensitive/replacement/private-key/token/user-path mutations.
...
- Six retained raw API response paths/endpoints/bytes/SHA-256 values and direct historical Git-object byte/hash results.
