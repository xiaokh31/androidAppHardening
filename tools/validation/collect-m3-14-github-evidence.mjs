...
const WORKFLOW_PATH = ".github/workflows/m3-13-startup-attribution.yml";
const ARTIFACT_NAME = "m3-13-startup-attribution-raw";
const DIAGNOSTIC_BRANCH = "feat/m3-14-successor-startup-diagnostic";
const MAX_PAGE_BYTES = 4 * 1024 * 1024;
const MAX_ARTIFACT_BYTES = 64 * 1024 * 1024;
const MAX_ENTRY_BYTES = 16 * 1024 * 1024;

function fail(message) { throw new Error(`M3-14 evidence collection failed: ${message}`); }
...
function safeNewPath(value, label) {
  ...
  const build = path.join(root, "build", "m3-14");
  fs.mkdirSync(build, { recursive: true });
  const resolved = path.resolve(value);
  if (!(resolved + path.sep).startsWith(build + path.sep) || fs.existsSync(resolved)) fail(`${label} must be new under build/m3-14`);
...
function headers(accept, token = true) {
  const value = { Accept: accept, "User-Agent": "androidAppHardening-M3-14-evidence", "X-GitHub-Api-Version": "2022-11-28" };
...
async function main() {
  ...
  if (requestPath !== path.resolve("docs/evidence/M3-14/diagnostic-terminal-request.json")) fail("request path differs");
  const request = JSON.parse(fs.readFileSync(requestPath, "utf8"));
  exactKeys(request, ["schemaVersion", "taskKey", "productTuple", "executionIdentitySha256", "diagnosticHeadSha", "diagnosticRunId"], "request");
  if (request.schemaVersion !== 1 || request.taskKey !== TASK_KEY || request.productTuple !== PRODUCT_TUPLE ||
      !/^[0-9a-f]{64}$/u.test(request.executionIdentitySha256) ||
  ...
  try {
    ...
    if (ledger.executionIdentitySha256 !== request.executionIdentitySha256 ||
        ledger.executionIdentity?.productTupleSha256 !== request.productTuple || ledger.taskKey !== request.taskKey ||
        ledger.workflowName !== RUN_NAME) fail("artifact ledger differs from the committed request");
    const pageFiles = {
