...
- `assets/ah/runtime/payload.ahdc` 对应的 AHDC v1 builder、只读 verifier、不可变描述模型和 `KeyPackagingPlan`。
...
- little-endian header、record table、认证字段和 ciphertext 的序列化。
...
- header 固定包含 major、minor、header size、flags、DEX count、record table size、payload size、16-byte build ID、16-byte key slot ID 和 32-byte manifest MAC；record 按原 `classes.dex`、`classes2.dex` 顺序递增。
- 每条 record 固定包含 ordinal、规范 DEX name、原始明文大小、ciphertext 大小、payload offset、12-byte nonce 和 32-byte plaintext SHA-256；每段 payload 为 ciphertext 紧跟 16-byte GCM tag，offset 必须递增、无重叠、无空洞并落在文件边界。
...
- manifest key 为 `HKDF-SHA-256(CEK, buildId, "AHDC manifest v1")`，长度 32 bytes；manifest MAC 使用 HMAC-SHA-256 覆盖固定 header 与完整 record table，计算时 MAC 字段置零。
- 每 DEX 使用独立随机 12-byte nonce 和 AES-256-GCM 128-bit tag；builder 检测本容器 nonce 重复并重新生成，连续三次冲突则失败。
- 每条 GCM AAD 固定为 magic、major、minor、build ID、key slot ID 与该条 canonical record；parser 完整消费文件并拒绝任何尾随 byte。
- 按 ADR-0006 为每次运行随机生成 32-byte `R` 与 `R_java`，计算 `R_native=R XOR R_java`；`KEK=HKDF-SHA-256(R, buildId, "AHDC offline KEK v1" || signerSha256)`。
- CEK 使用独立随机 12-byte nonce 和 AES-256-GCM 包装，AAD 为 container major、build ID、key slot ID 与 signer SHA-256。`KeyPackagingPlan` 将 wrapped CEK/public binding、Java share 和每 ABI Native share 分配给 config/bootstrap/Runtime materializer，任何单一静态位置不得含完整 `R`。
...
- `ContainerBuildResult` 包含 `descriptor` 与 `keyPackagingPlan`；后者仅能被 Runtime assembler 消费并在使用后销毁。
- `DexContainerDescriptor` 包含版本、package、signer digest、DEX 顺序/大小/摘要、container SHA-256，不暴露 key、nonce 之外的恢复材料或明文。
...
5. 对 magic、version、flag、count、length、offset、build ID、key slot ID、manifest MAC、wrapped CEK/AAD、nonce、tag 和 ciphertext 的单 bit 篡改均返回规定错误且不输出任何 DEX。
...
- header/record/ciphertext 全字段 tamper matrix。
