...
function runSelfTests(documentBundle) {
  ...
  const documentMutations = [
    ["m305_dependency_removed", bundle => { bundle.index = bundle.index.replace(", M3-09, M3-13 |", ", M3-13 |"); }],
...
function validateDocuments(bundle) {
  ...
  requirePhrases(bundle.index, ["| M3-09 | [#68]", "M3-08 → M3-09 → M3-11 → M3-12 → M3-10 → M3-13 → separately authorized successor implementation → M3-05", ", M3-09, M3-13 |"], "task index");
