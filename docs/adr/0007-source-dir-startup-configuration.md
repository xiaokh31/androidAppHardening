...
7. 从同一 Native handle 生成无秘密、不可变的 `AuthenticatedPayloadMetadata`；常量时间比较其 `package_name_sha256` 与 Framework package 重算值、当前 signer，并将其 lineage 与实测 lineage 按旧到新顺序逐项等值比较；随后才允许创建 payload loader。
