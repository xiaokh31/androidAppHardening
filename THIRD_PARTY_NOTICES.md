...
| dexlib2 | `2.5.2` | Maven Central `org.smali:dexlib2`; upstream `JesusFreke/smali` archived read-only repository | BSD-3-Clause | M3-14 only verifies the retained authorized profile DEX files and constructs ignored scratch mutation copies in `host:container` test source; it cannot regenerate or sign profiles and does not enter Host, Runtime, CLI or distribution artifacts; Issues #70 and #82 |
