...
The future diagnostic is first-and-only for its exact task. Its canonical workflow path is `.github/workflows/m3-09-startup-attribution.yml`, task key is `M3-09-DIAGNOSTIC-V1`, and `runAttempt` must equal `1`. Before accepting a report, the executing workflow archives a GitHub API enumeration of every run for that canonical workflow and filters the exact head plus original baseline/protected product-APK tuple; failed, cancelled and no-artifact runs remain in the enumeration. Exactly one matching run must exist and its run ID must equal the current report. Changing workflow path, job name or artifact name cannot create a new eligibility tuple. The report binds the enumeration hash, workflow path, head SHA, run ID, job ID, boot ID hash prefix, campaign/mode/sample ordinals, trace hash, raw-sample hash, original and instrumented APK hashes and cleanup result. An invalid or ineligible result cannot be replaced by a later run on identical product bytes. This rule is separate from and does not reopen the completed M2-10 diagnostic.

The future profile artifacts are derived from the original Release/R8 baseline and protected APKs under a deterministic post-build probe manifest. Evidence binds all four original/instrumented APK hashes and a structural diff manifest that permits only the fixed `p1..p15` common call sites plus protected `h0..h8`; manifest/resources/native libraries/security configuration and all non-probe instructions must remain equivalent. Baseline receives no synthetic Factory. The same job calibrates the observer outside retained startup samples; `calibrationP95Ns * maximumProtectedProbeCount` must be at most `5,000,000` ns in both campaigns. Probe overhead is never subtracted from samples. A structural-equivalence failure, changed security/lifecycle event sequence, probe-count/location drift or overhead above 5 ms forces `UNATTRIBUTED` and cannot select a production owner.
...
- A dedicated validator parses a complete synthetic diagnostic report, recomputes every adjacent stage, nine-owner per-ordinal reconciliation, nearest-rank P50, 300 ms reproduction, 30 ms/10%/50% eligibility and unique owner/`UNATTRIBUTED` result, and verifies profile equivalence plus first-and-only identity before accepting the static contract.
