...
| M0-03 | Gradle、Android、Native、CI 工具链 | M0-01 |
...
| ID | 交付 | 主要依赖 |
| --- | --- | --- |
| M1-01 | 不可信 APK 检查器 | M0-03 |
| M1-02 | signer 验证与身份策略 | M1-01 |
| M1-03 | 二进制 AXML 转换器 | M0-05, M1-01 |
| M1-04 | 版本化认证加密容器 | M0 interface freeze |
| M1-05 | APK 重打包、签名材料移除与对齐 | M1-03, M1-04 |
| M1-06 | CLI、JSON 报告与稳定错误码 | M1-01, M1-02, M1-05 |
...
| ID | 交付 | 主要依赖 |
| --- | --- | --- |
| M2-01 | Shell AppComponentFactory | M0-05, interface freeze |
| M2-02 | Native 解密与 InMemoryDexClassLoader | M1-04, M2-01 |
| M2-03 | 运行时 signer 和完整性校验 | M1-02, M2-02 |
| M2-04 | 四 ABI 构建与一致接口 | M0-03, M2-02 |
| M2-05 | 环境风险引擎 | M2-03 |
| M2-06 | 内存导出成本控制 | M2-02, M2-03 |

### 退出门禁

- API 29 最低边界和当前最高声明 API 均通过强制 ABI 用例。
...
| ID | 交付 | 主要依赖 |
| --- | --- | --- |
| M3-01 | Android fixture 集 | M0 interface freeze |
| M3-02 | 篡改、parser 模糊和残留扫描 | M1, M2 |
| M3-03 | Windows/Ubuntu 等价性 | M1 |
| M3-04 | API/ABI/组件启动矩阵 | M2, M3-01 |
| M3-05 | 大小、冷启动、内存基准与预算 | M3-04 |

### 退出门禁

- 所有强制矩阵项有 pass 或符合文档的不支持分类。
...
| ID | 交付 | 主要依赖 |
| --- | --- | --- |
| M4-01 | 安全、许可证与供应链复核 | M3 |
| M4-02 | Windows/Ubuntu 发布包 | M4-01 |
| M4-03 | 发布证据、兼容性与用户文档 | M4-02 |
