...
端到端启动归因按 ADR 0016 使用 common `p0..p15` 相邻时间线，并在 protected 的 `p0..p1` 内使用 `p0,h0,h1..h7,h8,p1` 嵌套时间线；`h1..h7` 保留 M2-10 `t0..t6` 语义。所有时间戳必须来自同一 boot-time monotonic trace clock。baseline/protected 按保留样本 ordinal `1..15` 一一配对，每个 ordinal 重算 Runtime、pre-Application residual 与 common `p1..p8` 七段共九个 signed owner contribution；该向量必须精确等于同 ordinal 的 `protected(p8-p0)-baseline(p8-p0)`，不得遗漏、重叠、重排或把负值丢弃。逐样本调和通过后才分别计算 total 与各 owner 的 nearest-rank P50；P50 非线性，owner P50 不得相加冒充 total。Runtime 只拥有 `h0..h8`，platform residual、生命周期、traversal 与 rendering 不得改名为 Runtime。后续独立诊断只允许 `kotlin-multidex`、同一 API 36 job/boot 的 A/B 反序、每 mode 5 次预热与 15 个保留样本；两 campaign 的 total P50 均必须超过 300 ms，且恰好一个 owner 同时满足正 P50 至少 30 ms、跨 campaign variation 至多 10%、占各自 total 至少 50%，否则固定为 `UNATTRIBUTED`，不设 tie-break。profile APK 必须绑定原始/插桩哈希、固定 probe manifest 与结构 diff，baseline 不得合成 Factory，校准总 probe 开销不得超过 5 ms 且不得从样本中扣除。唯一诊断固定 workflow、task key、`runAttempt=1`，归档并哈希包含失败/取消/无 artifact 在内的 exact-head/product tuple 全量 run 枚举，只接受当前唯一 run；无效结果不得替换。M2-10 run `32099991400` 不重跑、不替换；M3-09 不运行 benchmark、KVM 或 ARM。
