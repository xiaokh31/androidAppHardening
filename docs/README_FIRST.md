...
除 M0-01 空远程种子提交和 M0-02 预先批准的 `docs/m0-project-package` 引导流程外，一个任务卡对应一个 GitHub Issue、一个含任务 ID 的分支和一个 Pull Request。Agent 每次只领取一张任务卡，只修改该卡授权的文件和接口，不顺带完成相邻任务；任何后续任务不得援引引导例外。
