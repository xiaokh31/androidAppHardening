...
M0 基础建设与可行性验证以及 M1 Host 处理链已经完成。M1-01 输入检查、M1-02 signer policy、M1-03 Binary AXML 转换器、M1-07 AHDC v2 分块认证容器合同、M1-04 AHDC v2 Host 实现、M1-05 APK 重打包与对齐、M1-06 CLI 与 JSON 报告均已合并并通过双平台门禁。M2 已启动：独立前置任务 M2-07 已固定 Mbed TLS 4.1.1/TF-PSA-Crypto 1.1.1 Native 密码后端并完成最终 `main` 门禁；M2-02 已恢复并进入 Native AHDC v2 解密与内存 DEX loader 实现。生产 RuntimeBundle 与可发布端到端发行包仍属于后续任务。
...
| [M2-02](docs/tasks/M2-02-native-decrypt-and-inmemory-loader.md) | 进行中 | M2-07 与最终 `main` 门禁已通过；原冻结分支已恢复并合入最新 main |
