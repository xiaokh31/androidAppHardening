...
The ignored downloads are under `build/m2-04/remote/ed6b21b/`; no artifact or tool was downloaded to the C drive. A following evidence-only commit may inherit this KVM result only if its diff is limited to tracked documentation and HandOff state; Build and Governance must still pass that final PR head.
