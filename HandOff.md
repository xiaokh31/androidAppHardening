...
handoff_id: HO-20260816-023744
updated_at: 2026-08-16T02:37:44+08:00
updated_by: /root
state: active
...
- M3-05 remains active on `chore/m3-05-performance-benchmarks` with unique draft PR #63 linked to Issue #22. Correction `809a7d26e67185a515c3a9fcf4eeb619eaf708ee` and coordination head `193e124804d161b38ddf9c0fc82c2f15d5a84cdb` passed Governance `31895635883`, Ubuntu/Windows Build `31895635802`, and Host benchmark `31895635768`; both platforms passed two fixed runs, all budgets and the unchanged 10% repeatability gate. KVM `31895635822` exposed missing fixed-package visibility; `951a79a8cc63a304ecd59dac78f0540c6075fb0f` closed it, and replacement KVM `31896362140` then rejected AGP's default `.test -> main` wiring as `NOT-SELF-INSTRUMENTING`. Correction `143d7fe8a66e0517858d78d3bfeb71921b745197` enabled AGP's documented self-instrumenting property and embedded a copied form of the pinned/locked/checksummed Kotlin 2.4.10 stdlib solely into the independently installed test APK. KVM `31898342753` exposed a fully-drawn trace boundary, and `1eacabe01fd488918e5349aa83784b0feaabf371` closed the copied-stdlib configuration-cache capture. KVM `31899258583` passed API 29 job `95047313610` but proved a posted empty UI frame had no associated RenderThread frame. Correction `e07c4a0004c8c7014fa27c456d5586bf12db1777` bound `reportFullyDrawn()` to the decor's first actual pre-draw traversal. KVM `31899867769` completed all 30 `java-single-dex/baseline` samples but exposed API 36's missing legacy final instrumentation code; `e0f8e5a005a688cf793bb1f2861089e5d13fb066` replaced that brittle marker with fresh-result plus explicit JUnit success validation. Exact-head KVM `31900781249` then completed the same 30 traces and reached the test's post-measurement code, where a redundant provider query failed because Macrobenchmark had already stopped the target process. The current bounded correction retains all 30 observations captured inside their samples, requires all protected observations to be LOW/ALLOW, and removes only the redundant post-measurement query. Because one observed mode takes about five minutes, the M3-05 branch receives a 45-minute device-step cap and 60-minute job cap while every other branch retains 30/45 minutes; cleanup remains unconditional. Local Android-test compilation, runner marker self-test, governance, M3-07 10-surface/20-report mutations and diff checks pass. No threshold, sample count, product Runtime, HIGH trigger, visible fixture content or public API changes. One exact-head API 36 replacement remains; Build/Governance pass, Windows Host passes, and the failed Ubuntu Host cell is being rerun once after a 10.23% p95 RSS repeatability observation. Exactly one API 29 ARM64 physical run follows only if API 36 is stable. No local emulator will be started. `stash@{0}` remains a recovery backup until merge.
