...
同一机器锁还固定 `runtime/native/src/main/cpp/ah_crypto_config.h` 的 SHA-256 `75094e9ef8dbb381cfadddc7e7c89eed9c622d471331ce95a30574e93156ef35`，以及四 ABI 未剥离 Release ELF 中恰好十二个 TF-PSA 内部本地符号。TF-PSA 的 PSA 初始化要求 built-in CTR-DRBG/entropy；GCM/DRBG 因此保留隐藏的 `mbedtls_aes_crypt_ecb` block 实现。该内部集合不启用 PSA ECB/CBC、padding 或随机生成 facade，也不得动态导出；CI 对集合的任何缺少、增加或顺序无关差异均失败关闭，并要求重新执行漏洞可达性审查。
