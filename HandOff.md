...
handoff_id: HO-20260815-023652
updated_at: 2026-08-15T02:36:52+08:00
updated_by: /root
state: active
...
Resume M3-02 and PR #52 on the merged M2-08 parser-bounds fix, run exactly one bounded existing CI matrix, and close M3-02 without repeating completed local or device work.
...
- M3-02 is active on `chore/m3-02-tamper-fuzz`, linked only to Issue #19 and draft PR #52. Final local implementation freeze `90ef2ecf662371c82fed5f3d0fa92dbf9324e9e2` adds the generated 69-case catalog, real unsigned APK/Binary AXML corpus and binary regressions, Jazzer `0.29.1` APK/AXML targets, Clang `18.1.3` libFuzzer + ASan/UBSan, exact resource/runner locks, five-target fail-closed aggregation, and API 29/36 named Runtime mutation evidence. Final bounded Host validation passed in 1m24s without a device, and the final independent review is `P0=0/P1=0/P2=0`. The former Native crash blocker is closed by M2-08; the branch now incorporates `main@622c352f4714010fbba751c38245cec60d0c03b8` before one exact-head CI matrix resumes.
- The first resumed PR #52 fuzz run `31828524638` exposed only a CI resource-configuration defect: Ubuntu Jazzer instrumented the full third-party classpath and hit the unchanged 2 GiB libFuzzer RSS ceiling in APK/AXML jobs. The bounded fix restricts coverage and hook instrumentation to repository-owned parser/fuzz packages, keeps every duration/input/RSS limit fail-closed, and passed the static lock plus a five-second offline APK/AXML smoke at about 550 MiB AXML RSS. Replacement run `31829163554` then failed before fuzzing because its literal Windows list separator was invalid on Linux; the final argument uses Gradle's platform separator without changing coverage or limits. A final replacement exact-head matrix is required; no extra local long run or device run is authorized.
...
| M3-02 | `/root` | `chore/m3-02-tamper-fuzz` | in_progress | M1-03, M1-04, M1-06, M2-02, M2-03, M2-06, M3-01 | 实现与独立复核全零已冻结；等待发布授权后创建唯一草稿 PR 并运行一次 exact-head 远端矩阵 |
...
1. Commit and push the bounded Jazzer instrumentation-scope correction to PR #52 without rewriting existing history.
2. Run one replacement exact-head M3-02 matrix: Ubuntu/Windows fuzz, API 29/36 KVM, Build and Governance; do not repeat completed local or physical-device work.
3. Fix only genuine PR #52 CI regressions, then record evidence, merge with expected-head protection and synchronize README/task/HandOff. Do not start M3-03/M3-04/M3-05 early.
