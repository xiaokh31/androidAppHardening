...
- M3-04 设备事件报告保留 catalog 原始序列；API 29 若发生配置 relaunch，只接受一次按 fixture 固定的 Activity 重建后缀并标记 `configuration_relaunch=true`，乱序、额外重复、Provider/Application/worker 重建或其他 API 的同类重复均失败；
...
端到端启动归因按 ADR 0016 使用 common `p0..p15` 相邻时间线，并在 protected 的 `p0..p1` 内使用 `p0,h0,h1..h7,h8,p1` 嵌套时间线；`h1..h7` 保留 M2-10 `t0..t6` 语义。所有时间戳必须来自同一 boot-time monotonic trace clock，零时长相邻段保留为零值。baseline/protected 按保留样本 ordinal `1..15` 一一配对，每个 ordinal 重算 Runtime、pre-Application residual 与 common `p1..p8` 七段共九个 signed owner contribution；该向量必须精确等于同 ordinal 的 `protected(p8-p0)-baseline(p8-p0)`，不得遗漏、重叠、重排或把负值丢弃。逐样本调和通过后才分别计算 total 与各 owner 的 nearest-rank P50；P50 非线性，owner P50 不得相加冒充 total。Runtime 只拥有 `h0..h8`，platform residual、生命周期、traversal 与 rendering 不得改名为 Runtime。后续独立诊断只允许 exact `java-single-dex` pair、同一 API 36 job/boot 的 A/B 反序、每 mode 5 次预热与 15 个保留样本；两 campaign 的 total P50 均必须超过 300 ms，且恰好一个 owner 同时满足正 P50 至少 30 ms、跨 campaign variation 至多 10%、占各自 total 至少 50%，否则固定为 `UNATTRIBUTED`，不设 tie-break。PR #63 原始证据的 application delta P50 为 `331/432 ms`，但约 `30.5%` variation 失败，因此只作为最终拒绝与诊断来源，不称为稳定发布基线。M3-11 将 original baseline/protected 固定为 artifact `9260244215` 中 SHA-256 `4607d3289e1fc3bd95282ab47791ec810a5d2d3ac0a69fc0f91388901e412dcf` 与 `1eb159d7f0149a943fb2e1c4d8467f283d1cfbbfad670628402cfb0cd23390d9` 的实际签名 APK，并直接解析 manifest、两份 campaign report 与 repeatability aggregate 证明该映射；源码重建、重签、ZIP normalization 或 container 再生成均不是同一 original。M3-12 将已复核 profile package 固定到 release ID `374769776`、asset ID `524507375`、archive SHA-256 `21816d2a843bb5c59902224c7bf786d546d52b4a5b2d1168ca0c449a2ca27964` 和十个 member 的精确哈希；GitHub `immutable=false`，因此必须以只读权限按 numeric asset ID 下载并在任何 emulator/install 前验证，删除、替换或不可用均失败关闭且不得再生。M3-10 必须先直接哈希 exact originals 与 profile asset，再实现读取真实 APK/DEX/Release 产物与 A/B 原始 calibration/event 文件的字节级 verifier；自报等价布尔值不构成证据。若 exact bytes 不可用或 profile 派生需要已删除的临时私钥，任务失败关闭，不得用 rebuilt fixture 替代。诊断 workflow 只产出 raw package，终态后由独立 evidence workflow 读取并哈希官方 runs/jobs/artifacts API 原始分页，验证唯一 run、job、artifact、`runAttempt=1` 和预先固定的 task/product tuple；无效结果不得替换。M3-09 的当前 validator 仅验证合成合同模型，固定 `contractModelOnly=true`、`realEvidenceAccepted=false`，不运行 benchmark、KVM 或 ARM，也不声称验证真实产物。M2-10 run `32099991400` 保持终态，不重跑、不替换。

M3-10 run `32554806537` 在 provenance preflight 中失败并具有 zero device observation：未创建 AVD、未尝试安装、未保留样本且 artifact count 为零。ADR 0018 只允许在官方 API/步骤/日志与 reviewed workflow bytes 共同证明该边界时固定一个不同的 successor execution identity。它必须绑定全历史 qualification、reviewed implementation/candidate workflow/runner/verifier/lock hashes，并在任何 Android setup 前复核；改名 workflow 或 task 不能产生资格。Successor 无论成功、`UNATTRIBUTED`、取消或失败均终结资格，no further renewal；ARM、API 29、真机和完整 M3-05 矩阵不属于该诊断。
