    ...
    windows: {
      ...
      reviewed_images: [
        ...
        },
        {
          ...
          cl_runtime_version: "19.51.36256",
        },
      ],
...
function verifyWindowsWorkflowBinding(text) {
  const required = [
    ...
    "$escapedClRuntimeVersion = [regex]::Escape([string] $selectedImage.cl_runtime_version)",
    "cl_runtime_version=$($selectedImage.cl_runtime_version)",
...
if (selfTest) {
  ...
  const mutations = [
    ...
    ["Windows unreviewed image addition", (candidate) => { candidate.ci_toolchains.windows.reviewed_images.push({ image_version: "20990101.1.1", manifest_ref: "win25-vs2026/20990101.1", visual_studio_version: "changed", visual_studio_x64_tools: "changed", cl_runtime_version: "changed" }); }],
    ...
    ["Windows cl runtime", (candidate) => { candidate.ci_toolchains.windows.reviewed_images[3].cl_runtime_version = "changed"; }],
  ...
  const windowsWorkflowMutations = [
    ...
    ["Windows per-image cl runtime binding", (candidate) => candidate.replace(
      "$selectedImage.cl_runtime_version",
      "$toolchainLock.cl_runtime_version",
    )],
