...
Independent review 7 rejected frozen SHA `4a4de2820308eb7adefa4f5cfbc76eed0dbb6031` with `P0=0/P1=0/P2=2`, archived in `read-only-review-7.md`. Exact-head Build `31147638536`, Governance `31147638532` and KVM `31147638429` were green and the actual four ABI surfaces remained exact, but Ubuntu discarded non-`t/d` related symbols before set comparison while its broad export scan did not cover unprefixed `ctr_drbg_`/`entropy_`; a future uppercase/global helper could evade both. Remediation centralizes parsing for Ubuntu and Windows, collects all related names regardless of type, separately rejects related dynamic exports, and self-tests extra local/global/hidden-global, missing, type-change and export mutations. The workstream checkpoint becomes conditional rather than future-tense. A new SHA, replacement exact-head gates and eighth full independent review remain mandatory.

At `2026-08-07T12:53:24+08:00`, Node syntax, shared parser self-test, the dependency lock self-test, governance and `git diff --check` exited `0`. The shared parser accepted each current ABI's exact seventeen-entry surface and zero related exports. Its in-process negatives rejected extra lowercase local, uppercase global/hidden-global, missing, `t↔d`, `t→T` and related dynamic-export cases before any CI claim. No device or emulator was started.

Independent review 8 rejected frozen SHA `dfda35fad30d7edf2ee1fdfe26d3248dacd15e91` with `P0=0/P1=0/P2=1`, archived in `read-only-review-8.md`. All nineteen prior findings and exact-head Build `31148877894`, Governance `31148877818` and KVM `31148877817` passed. The sole evidence-only gap was that the in-process matrix implemented `t→d` but not the reverse mutation of the only locked data symbol, despite describing this as `t↔d`; independent synthetic input confirmed the comparator already rejected `d→t`. Remediation adds that exact `d mbedtls_platform_dev_random` to `t mbedtls_platform_dev_random` case without changing accepted input, dependency, facade, ABI or runtime behavior. A new SHA, replacement exact-head gates and ninth full independent review remain mandatory.

At `2026-08-07T13:11:54+08:00`, Node syntax, the expanded shared parser self-test, dependency lock self-test, governance and `git diff --check` exited `0`. The parser self-test now explicitly rejects both `t→d` and `d→t` mutations in addition to the existing type, presence and export cases. Strict HandOff correctly rejected the still-dirty pre-commit tree; it must pass after the remediation is committed cleanly.

Frozen remediation commit `7190a6ee61285fe065d5de6cbe836a648482d658` passed exact-head Build `31149909030`, Governance `31149909021` and API 29/36 KVM `31149909014`. The ninth and final independent read-only review returned `P0=0/P1=0/P2=0`, closed all twenty prior findings and introduced no new finding; its immutable conclusion is archived in `read-only-review-9.md`. This review is final unless a real acceptance-surface or security-boundary defect is demonstrated.
