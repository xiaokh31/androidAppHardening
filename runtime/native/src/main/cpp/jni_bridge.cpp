...
void throwCode(JNIEnv* environment, const char* code) noexcept {
    ...
    if (constructor != nullptr && message != nullptr) {
        jobject exception = environment->NewObject(type, constructor, message);
...
Java_ah_runtime_loader_NativePayloadBridge_nativeOpenVerifiedPayload(
    ...
    if (opened != ah::payload::Status::kSuccess) {
        throwCode(environment, payloadCode(opened));
        return 0;
    }
...
Java_ah_runtime_loader_NativePayloadBridge_nativeClosePayload(
    ...
    if (handle > 0) {
        (void) ah::handles::close(static_cast<std::uint64_t>(handle));
    }
}
