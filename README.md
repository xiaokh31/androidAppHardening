...
M2-07 Ubuntu runner 精确锁维护已关闭先前工具链阻塞；PR #74 的 exact-head 及合并后 `main` Build 均在 Ubuntu/Windows 通过。GitHub 后续分配的 Windows `20260818.207.1` 已由 Issue #77 独立进入 runtime/ref 与逐镜像 VS/x64 tools/`cl.exe` 固定流程；PR #78 首次 Build 精确发现新镜像 `cl.exe 19.51.36256`，替换候选必须独立复核并通过双平台 Build/Governance，期间 PR #76 保持失败关闭，不通过重跑或豁免绕过。
