...
- `public final class RiskReportV1`，构造时复制不可变的 `List<RiskSignal>`，并公开 `version`、`level` 与 `totalScore` 只读访问器。
- `public final class EnvironmentRiskEngine`，通过静态方法 `public static RiskReportV1 evaluate(Context context)` 评估风险并禁止实例化。
...
- 注入映射加调试器场景达到 `HIGH` 并启用 M2-06 高等级策略；单个 debuggable 信号保持 `LOW`。
