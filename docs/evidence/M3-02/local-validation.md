...
- Cross-platform correction: replacement run `31829163554` proved the literal Windows `;` list separator was rejected by Jazzer on Ubuntu before fuzzing. The argument now uses Gradle `File.pathSeparator` (`;` on Windows, `:` on Linux); the exact include patterns and all resource limits are unchanged.
- Linux pool correction: final replacement run `31829589792` correctly entered AXML fuzz but reported a `2159 MiB` fixed startup RSS on Ubuntu. The 2 GiB cap remains unchanged; heap/metaspace/code/direct/stack pools are reduced from `256/384/128/128 MiB + 512 KiB` to `128/192/64/64 MiB + 256 KiB`, with Serial GC, preserving roughly 330 MiB additional headroom for Jazzer's Linux-native instrumentation.
- KVM fixture correction: exact-head KVM `31829963762` failed before emulator startup because the shared generator applied M3-02 authenticated-container nonce/tag/ciphertext mutations to the intentionally legacy M0-05 PoC container. The generator now emits those seven mutations only when the single payload is canonical AHDC v2; the later M2-03 production fixture still receives the complete matrix, while historical M0-05 retains its existing generic payload-corrupt case.
- Build report correction: Build `31830770680` produced byte-identical Ubuntu/Windows M1-03 reports; only the extended `error-matrix.json` lock was stale. Both workflow branches now lock the already recorded M3-02 hash `b287183d1c2af46cfb9ce4b027e7993ec9721e039f91c3125176a962a2ddd641`; transform, fuzz and aapt2 report hashes are unchanged.
- KVM report wiring correction: API 29 job `94865641953` completed the shared M202 loader acceptance as task `M2-06`, then the M3-02 summarizer failed only because the workflow referenced a nonexistent `build/m2-02/device-.../report.json`. The summary now consumes the report actually produced by the bounded M2-06 step and accepts task IDs `M2-02` or `M2-06` while retaining the exact 21-case, two-variant, cleanup and publication-field checks; no device command or acceptance threshold changed.
